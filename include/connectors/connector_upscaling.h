...
class UpscaleConnector : public Connector<TElem>
{
    ...
    void dimChecks(const std::vector<NodeShPtr<TElem>>& input_nodes) const
    {
        if(input_nodes.size() != 1) {
            throw std::invalid_argument("Need exactly one input for average pooling");
        }
    }
                ...
                for(size_t i_pool = 0; i_pool < _pool_width; i_pool++) {
                    ...
                    for(size_t j_pool = 0; j_pool < _pool_height; j_pool++) {
                        ...
                        for(size_t out_chan = 0; out_chan < n_channels; out_chan++) {
                            out_view(i * _pool_width + i_pool, j * _pool_height + j_pool,
                                     out_chan) += weight * input(i, j, out_chan);
                        }
                    }
                }
            }
            ...
            for(size_t j = 0; j < image_height; j++) {
                ...
                for(size_t i_pool = 0; i_pool < _pool_width; i_pool++) {
                    ...
                    for(size_t j_pool = 0; j_pool < _pool_height; j_pool++) {
                        ...
                        for(size_t out_chan = 0; out_chan < n_channels; out_chan++) {
                            input_grad(i, j, out_chan) +=
                                weight * out_grad_view(i * _pool_width + i_pool,
    ...
    UpscaleConnector(size_t pool_width, size_t pool_height)
...
public:
    virtual ~UpscaleConnector() {}
};

template<class TElem>
NodeShPtr<TElem> Upscale2D(const NodeShPtr<TElem>& node, size_t pool_height, size_t pool_width)
{
    auto conn = Connector<TElem>::template create<UpscaleConnector>(pool_height, pool_width);
    {
