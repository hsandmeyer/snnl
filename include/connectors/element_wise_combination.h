...
public:
    ...
    outputDims(const std::vector<NodeShPtr<TElem>>& input_nodes) const override
    {
        ...
        if (input_nodes.size() != 2) {
            throw std::invalid_argument(
                "Exactly two nodes needed for element wise combination");
        }
                        ...
                        Node<TElem>* output_node) override
    {
        ...
        if (a.NDims() > b.NDims()) {
            Tensor<TElem> out_view =
                output_node->values().reshapeFromIndices({-b.NDims()});
            ...
            for (size_t i = 0; i < a_view.shape(0); i++) {
                for (size_t j = 0; j < a_view.shape(1); j++) {
                    out_view(i, j) =
                        Functor<TElem>::forward(a_view(i, j), b_view(j));
                }
            }
        }
        else {
            Tensor<TElem> out_view =
                output_node->values().reshapeFromIndices({-a.NDims()});
            ...
            for (size_t i = 0; i < b_view.shape(0); i++) {
                for (size_t j = 0; j < b_view.shape(1); j++) {
                    out_view(i, j) =
                        Functor<TElem>::forward(a_view(j), b_view(i, j));
                }
            }
        }
    }
                         ...
                         std::vector<NodeShPtr<TElem>>& input_nodes) override
    {
        ...
        if (val_a.NDims() > val_b.NDims()) {
            ...
            Tensor<TElem> a_val_view =
                val_a.reshapeFromIndices({-val_b.NDims()});
            Tensor<TElem> b_val_view = val_b.shrinkToNDimsFromLeft(1);

            Tensor<TElem> a_grad_view =
                grad_a.reshapeFromIndices({-grad_b.NDims()});
            ...
            for (size_t i = 0; i < a_val_view.shape(0); i++) {
                for (size_t j = 0; j < a_val_view.shape(1); j++) {
                    auto [deriv_a, deriv_b] = Functor<TElem>::backward(
                        a_val_view(i, j), b_val_view(j));
            ...
            Tensor<TElem> b_val_view =
                val_b.reshapeFromIndices({-val_a.NDims()});
            Tensor<TElem> a_val_view = val_a.shrinkToNDimsFromLeft(1);

            Tensor<TElem> b_grad_view =
                grad_b.reshapeFromIndices({-grad_a.NDims()});
